...
% ### Feedback control computation and update (using quarternions)    ###
% ###                                                                 ###
...
function [ ATT_BR,   ...
           ...
           OHM_BN,   ...
           INTG_ERR, ...
           CTRL_TORQ ] = ctrl_feedback_qrt( TIME,     ...
                                            STEPD,    ...
                                            STEPC,    ...
                                            Kp,       ...
                                            Ki,       ...
...
if dot( ATT_BN, ATT_BN ) ~= 1.0
    ATT_BN = ATT_BN / norm( ATT_BN );
end

% Initialise the reference-to-body and reference-to-inertial attitudes.
DCM_BN = qrt_conv2dcm( ATT_BN );
DCM_BR = DCM_BN * DCM_RN';
ATT_BR = dcm_conv2qrt( DCM_BR );

% Second check: Ensure that the quarternions always have norm = 1.
if dot( ATT_BR, ATT_BR ) ~= 1.0
    ATT_BR = ATT_BR / norm( ATT_BR );
end
...
if mod( TIME, STEPC ) == 0.0
    ...
    if CTRL == 1
        CTRL_TORQ = ctrl_response_PD( Kp, Ki, Kd, ATT_BR(2:4), OHM_BR );
        
    % Mode 2: Trigger the Lyapunov PID Control Response
    elseif CTRL == 2
        CTRL_TORQ = ctrl_response_PID( Kp, Ki, Kd, ATT_BR(2:4), OHM_BR, ...
                                       INTG_ERR(2:4) );
        
    % Mode 3: Trigger the Model Predictive Control Response
    elseif CTRL == 3
        CTRL_TORQ = ctrl_response_MPC( Kp, Ki, Kd, ATT_BR(2:4), OHM_BR );
        
    % Default Mode: Lyapunov PD Control Response
    else
        CTRL_TORQ = (-Kd * ATT_BR(2:4) ) - ( Kp * OHM_BR ); 
    end
...
ATT_BN_DOT = qrt_derivative( ATT_BN, OHM_BN );
