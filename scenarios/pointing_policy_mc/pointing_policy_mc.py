...
fname = "montecarlo_run01.txt"
mc_time_plot  = []
mc_total_time = 0.0 # Total time of simulation
mc_num_iters  = 1000

for iteration in range(0, mc_num_iters):
    ...
    with open( deputy_path ) as deputy_elements:
        ...
        for row in deputy_elements_csv:
            ...
            if 'Deputy' in name:
                ...
                # Randomly initialise a deputy spacecraft.
                # Start with the chief spacecraft for starting parameters.
                aC, eC, iC, wC, RC, MC = sC.a, sC.e, sC.i, sC.w, sC.R, sC.M
                
                # Generate a random radial, in-track, cross-track variations
                # for a new deputy.
                fR = np.random.random() * 4 + 1 # Random number between 1 to 5 km.
                fI = 2*fR                       # Twice of fR, as per HCW equation.
                fO = 0.0                        # None.
                fC = np.random.random() * 4 + 1 # Random number between 1 to 5 km.
                fPhi = np.random.random() * 360.0 - 180.0
                fTht = np.random.random() * 360.0 - 180.0
                
                a, e, i, w, R, M = deputy.deputy( aC, eC, iC, wC, RC, MC,
                                                  fR, fI, fO, fC, fPhi, fTht)
    ...
    pdrain  = 0.000                       # Power drain rate, between 0 and 1
    ...
    mc_total_time += t
    mc_time_plot.append(t)

fout = open(fname, "w")
for ttt in mc_time_plot:
    fout.write(str(ttt) + "\n")
fout.close()

plt.scatter( list(range(len(mc_time_plot))), mc_time_plot )
